...
func GetLikedPosts(w http.ResponseWriter, r *http.Request, db *sql.DB) {
	...
	posts, statusCode, err := FetchLikedPosts(db, username, offset)
	if err != nil {
...
func FetchLikedPosts(db *sql.DB, username string, offset int) ([]models.Post, int, error) {
	if username == "" {
    ...
    INNER JOIN 
        users liker ON pr.user_id = liker.id
    WHERE 
        liker.username = ? 
	...
	if len(posts) == 0 {
		return nil, 404, fmt.Errorf("no liked posts found for user %s", username)
	}
